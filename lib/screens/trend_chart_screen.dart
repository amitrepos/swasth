...
class TrendChartScreenState extends State<TrendChartScreen>
  ...
  Future<void> _loadSummary(int period) async {
    ...
    try {
      ...
      final summary = await _readingService.getTrendSummary(
        token,
        widget.profileId,
        period,
      );
...
class _TrendView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final glucose = filtered
        .where((r) => r.readingType == 'glucose' && r.glucoseValue != null)
        .toList();
    final bp = filtered
        .where((r) => r.readingType == 'blood_pressure' && r.systolic != null)
        .toList();
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Unconditional cards below: each type always renders its card. If no
            // readings exist for that type in the current window, the card body
            // shows an empty-state message instead of being silently hidden.
            // Prior behavior (hiding the entire card) made partial data look like
            // a broken feature — screenshot report 2026-04-18.

            // ── Glucose detail ───────────────────────────────────────────
            _DetailChartCard(
              key: const Key('trend_card_glucose'),
              icon: Icons.water_drop,
              color: _kGlucoseColor,
              title: l10n.glucoseTrend,
              unit: 'mg/dL',
              child: glucose.isNotEmpty
                  ? Column(
                      children: [
                        _GlucoseChart(
                          readings: glucose,
                  ...
                  : _EmptyReadingState(
                      key: const Key('trend_empty_glucose'),
                      icon: Icons.water_drop_outlined,
                      message: l10n.noReadingsInWindowGlucose(days),
                    ),
            ),
            const SizedBox(height: 16),
            ...
            _DetailChartCard(
              ...
              child: bp.isNotEmpty
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 20),
                            _LegendDot(
                              color: AppColors.statusNormal.withValues(
                                alpha: 0.5,
                              ),
                              label: 'Normal range',
                              isBar: true,
                  ...
                  : _EmptyReadingState(
                      key: const Key('trend_empty_bp'),
                      icon: Icons.favorite_outline,
                      message: l10n.noReadingsInWindowBp(days),
                    ),
            ),
            const SizedBox(height: 16),
            ...
            // ── Weight detail ───────────────────────────────────────────
            _DetailChartCard(
              key: const Key('trend_card_weight'),
              icon: Icons.monitor_weight_outlined,
              color: _kWeightColor,
              title: 'Weight Trend',
              unit: 'kg',
              child: weight.isNotEmpty
                  ? Column(
                      children: [
                        _WeightChart(
                          readings: weight,
                  ...
                  : _EmptyReadingState(
                      key: const Key('trend_empty_weight'),
                      icon: Icons.monitor_weight_outlined,
                      message: l10n.noReadingsInWindowWeight(days),
                    ),
            ),
          ],
        ),
...
class _CorrelationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final r in glucose) {
      glucoseByDay.putIfAbsent(
        DateFormat('yyyy-MM-dd').format(r.readingTimestamp),
        () => r,
      );
    }
    ...
    final sharedDays = glucoseByDay.keys.toSet().intersection(
      bpByDay.keys.toSet(),
    );
    final bothNormal = sharedDays
        .where(
          (d) =>
              glucoseByDay[d]?.statusFlag == 'NORMAL' &&
              bpByDay[d]?.statusFlag == 'NORMAL',
        )
        .length;
    final total = sharedDays.length;
    final hasBoth = total > 0;
    final pct = hasBoth ? (bothNormal / total * 100).round() : 0;
    final insightColor = pct >= 70
...
class _MiniLineChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final minY = (vals.reduce((a, b) => a < b ? a : b) - 15).clamp(
      0.0,
      double.infinity,
    );
...
class _MiniBpChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final allVals = [
      ...readings.map((r) => r.systolic!),
      ...readings.where((r) => r.diastolic != null).map((r) => r.diastolic!),
    ];
    final minY = (allVals.reduce((a, b) => a < b ? a : b) - 10).clamp(
      0.0,
      double.infinity,
    );
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              belowBarData: BarAreaData(
                ...
                gradient: LinearGradient(
                  colors: [
                    _kSysColor.withValues(alpha: 0.12),
                    _kSysColor.withValues(alpha: 0.0),
                  ],
                  begin: Alignment.topCenter,
                  end: Alignment.bottomCenter,
                ),
...
// ---------------------------------------------------------------------------
// Empty-state body for a DetailChartCard when no readings fell in the window.
// Replaces the old behavior of silently hiding the entire card, which made
// users think the feature was broken (screenshot report, 2026-04-18).
// ---------------------------------------------------------------------------
...
class _EmptyReadingState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    // Generous vertical padding so the empty state visually "claims" the chart
    // area — matches approximate height of a real chart so the layout doesn't
    // jump between tabs (7 vs 30 vs 90 days).
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Icon(
            icon,
            size: 36,
            // 50% (not 35%) — Aditya flagged that 35% fades out on budget LCDs
            // in sunlight, losing the visual anchor.
          ...
          const SizedBox(height: 6),
          Text(
            l10n.logFromHomeHint,
            textAlign: TextAlign.center,
            // 14sp is the Swasth elderly-accessibility floor for body text
            // (per Aditya). Hint is actionable ("Log from Home"), not
            // decorative — must be readable at arm's length without glasses.
            style: TextStyle(
...
class _GlucoseChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final vals = readings.map((r) => r.glucoseValue!).toList();
    final minY = (vals.reduce((a, b) => a < b ? a : b) - 20).clamp(
      0.0,
      double.infinity,
    );
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              belowBarData: BarAreaData(
                ...
                gradient: LinearGradient(
                  colors: [
                    _kGlucoseColor.withValues(alpha: 0.12),
                    _kGlucoseColor.withValues(alpha: 0.0),
                  ],
                  begin: Alignment.topCenter,
                  end: Alignment.bottomCenter,
                ),
...
class _BpChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final minY = (allVals.reduce((a, b) => a < b ? a : b) - 15).clamp(
      0.0,
      double.infinity,
    );
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              belowBarData: BarAreaData(
                ...
                gradient: LinearGradient(
                  colors: [
                    _kSysColor.withValues(alpha: 0.10),
                    _kSysColor.withValues(alpha: 0.0),
                  ],
                  begin: Alignment.topCenter,
                  end: Alignment.bottomCenter,
          ...
          lineTouchData: LineTouchData(
            touchTooltipData: LineTouchTooltipData(
              getTooltipItems: (spots) {
                final windowStart2 = DateTime.now().subtract(
                  Duration(days: days),
                );
                return spots.map((s) {
...
class _StatsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final normalPct = statuses.isNotEmpty
        ? (statuses.where((s) => s == 'NORMAL').length / statuses.length * 100)
              .round()
        : 0;
...
class _WeightChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final minY = (values.reduce((a, b) => a < b ? a : b) - 5).clamp(
      0.0,
      double.infinity,
    );
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              belowBarData: BarAreaData(
                ...
                gradient: LinearGradient(
                  colors: [
                    _kWeightColor.withValues(alpha: 0.15),

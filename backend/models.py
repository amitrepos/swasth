...
class User(Base):
    ...
    consent_timestamp = Column(DateTime(timezone=False), nullable=True)   # stored in user's local timezone (TIMESTAMP, not TIMESTAMPTZ)
    ...
    ai_consent_timestamp = Column(DateTime(timezone=False), nullable=True) # stored in user's local timezone (TIMESTAMP, not TIMESTAMPTZ)
    is_admin = Column(Boolean, default=False)
    timezone = Column(String, default="Asia/Kolkata", nullable=False)
    last_login_at = Column(DateTime(timezone=False), nullable=True)         # stored in user's local timezone (TIMESTAMP, not TIMESTAMPTZ)

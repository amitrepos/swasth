...
| P8 | **PII encryption at rest (Task E17)** | ✅ Shipped via **PR #165** (2026-04-24) as a single destructive migration (`0007_pii_encryption_batch.py`) — pre-pilot zero-user state allowed TRUNCATE + reseed instead of dual-write. PII now encrypted under `PII_ENCRYPTION_KEY` across `users`, `profiles`, `profile_invites`, `*_otps`, `doctor_profiles`. Runbook: `docs/runbooks/PII_ENCRYPTION_BATCH.md`. **Remaining:** verify `PII_ENCRYPTION_KEY` is set as env on the India server after P1. | Eng | Done | ✅ |

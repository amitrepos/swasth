...
| C25 | Caregiver Wellness Hub dashboard | ✅ Done | Behind `FeatureFlags.caregiverDashboard`. Shows when viewing shared profile (viewer/editor). Wellness Hub header, personalized messages, activity feed, 90-day trends, care circle, priority call. "Take Readings" toggle for in-person caregiving. PRs #75, #77, #79. |
| C26 | Care Circle widget | ✅ Done | Shows family members with role badges (Owner/Editor/Viewer), relationship, last active indicator, Call/WhatsApp/Email contact. Full-width Row layout. Visible on both owner and caregiver dashboards. PRs #77, #79, #80. |
| C27 | Manage Access UX improvements | ✅ Done | "PROFILE SHARED WITH" section header, better empty state, colored initial avatars, edit relationship dialog (tap member → dropdown), edit pencil hint. PR #81. |
| C28 | BMI in vitals grid | ✅ Done | Moved BMI from full-width row into 2x2 vitals grid (replacing SpO2 tile). Matches Steps tile shape/size. PR #74. |
